...
drop table pdf_calculator_results;

create table pdf_calculator_results (
  low_x        number constraint pdf_calculator_results_pk primary key,
  card         number,
  card_diff    number,
  card_stnd_f  number
);

create or replace procedure pdf_calculator (
...
begin
  delete from  pdf_calculator_results;
  commit;
  ...
  for i in 0..p_num_samples-1 loop
    ...
    insert into pdf_calculator_results (low_x, card, card_stnd_f, card_diff)
...
exec pdf_calculator (0.001, 0.1, 9.9-0.001, 101);
col low_x form 99.999999 
set echo off
set feedback off
spool range_sel_inf_02_data.dat
select * from pdf_calculator_results order by low_x;
spool off
...
set echo off
set feedback off
spool range_sel_inf_03_data.dat
select * from pdf_calculator_results order by low_x;
spool off
...
set echo off
set feedback off
spool range_sel_inf_04_data.dat
select * from pdf_calculator_results order by low_x;
spool off

set autotrace traceonly explain 
select x from t where x > 0.1 and x < 0.1 + 0.001;
select x from t where x > 0.1 and x < 0.1 + 2;
select x from t where x > 0 and x < 0+0.00000000000000001;
