...
create table ft_test (
  ...
  constraint ft_test_pk primary key (N,s)
);

drop table ft_perf;

create table ft_perf (N int not null, millis int not null);

create or replace procedure ft_test_p (
  p_min_N       int, 
  p_max_N       int, 
...
begin
  l_stmt 
  := 'insert into ft_test (N, s, probability, probability_exact)'
...
select cast (s as number) s, probability from p_s_N
  ...
  while N <= p_max_N loop
    ...
    insert into ft_perf (N, millis) values (N, (l_time_end - l_time_start) * 10);
    commit;
