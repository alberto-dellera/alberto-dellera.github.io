...
--
-- Perils of special cardinality
-- It shows how the "special cardinality" contributor can make
-- for a severe overestimation of the cardinality if the last value occurs frequently.
--
...
spool perils_of_special_cardinality.lst
...
-- table t1 with its Frequency histogram collected
create table t1 (value number);
insert into t1(value) select 10 from dual connect by level <= 2;
insert into t1(value) select 20 from dual connect by level <= 100;
...
select value, count(*) from t1 group by value order by value;
select * from formatted_hist_t1;

-- table t2 with its Frequency histogram collected
create table t2 (value number);
insert into t2(value) select  0 from dual connect by level <= 100;
insert into t2(value) select 20 from dual connect by level <= 2;
...
select value, count(*) from t2 group by value order by value;
