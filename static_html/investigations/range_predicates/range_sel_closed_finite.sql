...
drop table cdf_calc_closed_results;

create table cdf_calc_closed_results (
  low_x        number,
  high_x       number constraint cdf_calc_closed_results_pk primary key,
  card         number,
  card_diff    number,
  card_stnd_f  number
);
...
begin
  delete from  cdf_calc_closed_results;
  commit;
  ...
  for i in 1..p_num_samples loop
    ...
    insert into cdf_calc_closed_results (low_x, high_x, card, card_stnd_f, card_diff)
...
set echo off
set feedback off
spool range_sel_closed_finite_02_data.dat
select * from cdf_calc_closed_results order by low_x, high_x;
spool off
...
set echo off
set feedback off
spool range_sel_closed_finite_03_data.dat
select * from cdf_calc_closed_results order by low_x, high_x;
spool off
...
set echo off
set feedback off
spool range_sel_closed_finite_04_data.dat
select * from cdf_calc_closed_results order by low_x, high_x;
spool off
